...
public static class BoditrakUtils{

    public static Vector2 CalculateCOP(float[] weights){  
        ...
        if (sumWeights > 0){
            ...
            cop.x = (copX / 16.0f) - 1.0f; 
            cop.y = (copY / 16.0f) - 1.0f;
        }
...
public class BoditrakCOM : MonoBehaviour
{
    public ConcurrentQueue<float[]> COMQueue = new ConcurrentQueue<float[]>(); 
    public float[] weights = new float[1024];
    ...
    async void Update()
    {
        ...
        if(COMQueue.TryDequeue(out weights)){
            cop = BoditrakUtils.CalculateCOP(weights);
        }

        transform.localPosition = new Vector3(cop.x, cop.y, 0);
